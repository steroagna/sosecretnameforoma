...
//        	oldOF = bestTimetable.objFunc;            
            if (i < 7) {
                temperature = 1/(1/temperature + alfa);
                plateau = data.examsNumber * a;
                a += 2;
//                System.out.println("temp: " + temperature);
                for (int j = 0; j < plateau && i < 20; j++) {
...
//                    updateBest(timetable, data, true);                    
                    ...
                    // updateBest restituisce true se c'è stato miglioramento, se no false
                    // l'argomento true serve solo per quando stampo OF e vedere se il miglioramento 
                    // è dovuto a questa parte o a quella della kempe chain
                    if (updateBest(timetable, data, true))
                    	i=0;
                    else
                    	i++;
                }
//                if ((oldOF - bestTimetable.objFunc)/oldOF > epsilon)
//                {
//                	i = 0;
//                	oldOF = bestTimetable.objFunc;
//                }
...
//                	i++;
                ...
                temperature = setTemperature(timetable, rep, data);
...
//                    timetable = tempTimetable;
//                }
                if (updateBest(timetable, data, false))
                	i=0;
...
//            }
    ...
    private Timetable kempeChain(Timetable timetable, Data data, int k) {
    	//NB: dalla Kempe faccio sputare una nuova timetable solo se è la migliore in assoluto
...
//              bestTimetable = new Timetable(tempTimetable);
    ...
    private double setTemperature(Timetable timetable,  int rep, Data data) {
        ...
        Timetable startingTimetable;

        for (int i = 0; i<rep ; i++) {
//            tempTimetable = kempeChain(timetable, data, (int) Math.floor(data.slotsNumber/3));
        	startingTimetable = new Timetable(timetable);
        	Move move = generatesNeighbourSwappingExam(data, startingTimetable);
        	if (move.penalty > timetable.objFunc) {
        ...
        if (numberWorstSol > 0) {
            ...
            temperature = -((totPenalty - timetable.objFunc)) / Math.log(0.4);
        } else
            temperature = 1;
...
//                System.out.println("Timer: " + improvementTimer);
                System.out.println("OF? " + Util.ofCalculator(timetable, data) + flag);
...
//    private Move swapTwoExams(Data data, Timetable timetable) {
//
//        Move moving;
//
//        for(; ; ) {
